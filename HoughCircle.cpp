...
void EggsDetectorBind::run(void)
{
...
#if 1
    createTrackbar("Spatial window radius", windowName, &mSettings.mSpatialWindowRadius, 100, trackbarPropertyChanged,
                   this);
    createTrackbar("Color window radius", windowName, &mSettings.mColorWindowRadius, 2000, trackbarPropertyChanged,
                   this);
    ...
    createTrackbar("Accumulator Threshold", windowName, &mSettings.mAccumulatorThreshold, 100, trackbarPropertyChanged,
                   this);
...
#if 0
static void icvFindCirclesGradient(CvMat *img, Mat &contour_img, float dp, float min_dist, int min_radius,
                                   int max_radius, int low_threshold, int high_threshold, int acc_threshold,
                                   CvSeq *circles, int circles_max)
{
    ...
    for (y = 1; y < arows - 1; y++) {
        for (x = 1; x < acols - 1; x++) {
            ...
            if (adata[base] > acc_threshold && adata[base] > adata[base - 1] && adata[base] > adata[base + 1] &&
                             ...
                             int low_threshold, int high_threshold, int acc_threshold, int min_radius, int max_radius)
{
    ...
    if (CV_IS_STORAGE(circle_storage)) {
        circles = cvCreateSeq(CV_32FC3, sizeof(CvSeq), sizeof(float) * 3, (CvMemStorage *)circle_storage);
    } else if (CV_IS_MAT(circle_storage)) {
        mat = (CvMat *)circle_storage;

        if (!CV_IS_MAT_CONT(mat->type) || (mat->rows != 1 && mat->cols != 1) || CV_MAT_TYPE(mat->type) != CV_32FC3)
            CV_Error(CV_StsBadArg,
                     "The destination matrix should be continuous and have a single row or a single column");

        circles = cvMakeSeqHeaderForArray(CV_32FC3, sizeof(CvSeq), sizeof(float) * 3, mat->data.ptr,
                                          mat->rows + mat->cols - 1, &circles_header, &circles_block);
        circles_max = circles->total;
        cvClearSeq(circles);
    } else
        CV_Error(CV_StsBadArg, "Destination is not CvMemStorage* nor CvMat*");

    icvFindCirclesGradient(img, contour_image, (float)dp, (float)min_dist, min_radius, max_radius, low_threshold,
                           high_threshold, acc_threshold, circles, circles_max);
